...
def create_mask(
    width: int, height: int, mask_width: int, mask_height: int, x: Optional[int] = None, y: Optional[int] = None
) -> np.ndarray:
...
def stitch_images(inputs: np.ndarray, *outputs: Iterable, img_per_row: int = 2) -> np.ndarray:
    gap: int = 5
    columns: int = len(outputs) + 1

    width, height = inputs[0][:, :, 0].shape
    img = Image.new(
        "RGB", (width * img_per_row * columns + gap * (img_per_row - 1), height * int(len(inputs) / img_per_row))
    )
